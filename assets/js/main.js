(function ($) {
	...
	// IMPORTANT: Prevent menu from reverting to theme default
	// Remove or comment out any code that copies/clones the main menu HTML into the mobile menu.
	// For example, if you see something like this, REMOVE or COMMENT IT OUT:
	...
	/* Hero Slider Activation */
		...
		autoplay: {
			...
			disableOnInteraction: false, // continues even after user interaction
		},
		pagination: {
		...
		effect: 'slide',  // default transition effect
		});
...
(function($) {
  $(document).ready(function() {
    ...
    // 1. Home - ensure single item with correct link and text
    menu.find('li').filter(function() {
      return $(this).children('a').text().trim().toLowerCase().includes('home');
    }).each(function() {
      var li = $(this);
      li.children('ul').remove(); // remove dropdown if any
      li.children('a').attr('href', 'index.html').html('<span>Home</span>');
    });

    // 2. Shop - ensure single item "Shop Grid"
    menu.find('li').filter(function() {
      return $(this).children('a').text().trim().toLowerCase().includes('shop');
    }).each(function() {
      var li = $(this);
      li.children('ul').remove(); // remove dropdown
      li.children('a').attr('href', 'shop-grid.html').html('<span>Shop Grid</span>');
    });

    // 3. Pages - remove 404 error page link if exists
    var pagesLi = menu.find('li.has-children').filter(function() {
      return $(this).children('a').text().trim().toLowerCase() === 'pages';
    });
    if (pagesLi.length) {
      pagesLi.find('ul.sub-menu li').filter(function() {
        return $(this).text().toLowerCase().includes('404');
      }).remove();
    }

    // 4. Blog - ensure no dropdown, just "Blog" link
    menu.find('li').filter(function() {
      return $(this).children('a').text().trim().toLowerCase() === 'blog';
    }).each(function() {
      $(this).children('ul').remove();
      $(this).children('a').attr('href', 'blog.html').html('<span>Blog</span>');
    });

    // 5. Contact - do nothing (leave as is)

    // Optionally, remove any extra "Home 2" or other unwanted pages
    // (If "Home 2" or unwanted pages appear as siblings, remove them by name)
    menu.find('li').filter(function() {
      var text = $(this).children('a').text().trim().toLowerCase();
      return text === 'home 2' || text === '404 error' || text === 'some unwanted page';
    }).remove();

    console.log('Menu cleaned and structured.');
  });
})(jQuery);
